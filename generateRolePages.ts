// generateRolePages.ts
import fs from 'fs';
import path from 'path';
import { roles } from './src/data/roles'; // Adjust if your roles.ts is elsewhere

roles.forEach(role => {
  const roleDir = path.join(__dirname, 'src', 'app', 'career-builder', role.slug);
...
export const metadata = {
  title: '${role.title} Career Path - Pathsync Recruitment',
  description: '${role.summary}',
};

export default function RolePage() {
  const role = roles.find(r => r.slug === '${role.slug}');
  ...
  fs.writeFileSync(path.join(roleDir, 'page.tsx'), pageContent.trim());
});
